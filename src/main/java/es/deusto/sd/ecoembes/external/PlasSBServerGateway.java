...
public class PlasSBServerGateway implements IServiceGateway {
  ...
  public AssignmentExternalNotificationDTO assignDumpsterToPlant(
    ...
    try (var socket = new Socket(serverIP, serverPort);
      ...
      System.out.println(" - Sending AssignmentDTO to '" + socket.getInetAddress().getHostAddress() + ":"
          + socket.getPort() + "' -> '" + assignmentExternalNotificationDTO.getToken() + "'");

      try {
        assignmentExternalDTO_ret = (AssignmentExternalNotificationDTO) in.readObject();
      } catch (ClassNotFoundException e) {
      }
      System.out.println(" - Getting AssignmentDTO from'" + socket.getInetAddress().getHostAddress() + ":"
          + socket.getPort() + "' -> '"
          + (assignmentExternalNotificationDTO.getToken() != null ? assignmentExternalNotificationDTO.getToken()
              : "null")
          + "'");
